...
class Reservation {
  ...
  set numGuests(val) {
    if (val < 1) throw new Error("Not enough guest.");
    this._numGuests = val;
  }
  ...
  set customerId(val) {
    if (this._customerId && this._customerId !== val)
      throw new Error("Don't change the customer ID");
  ...
  static async get(id) {
    const result = await db.query(
      `SELECT id, customer_id AS "customerId", num_guests AS "numGuests", start_at AS "startAt",
           notes FROM reservations WHERE id = $1`, [id]);
    let reservation = results.row[0];
    if (reservation === undefined) {
      const err = new Error(`Reservation not found ${id}`);
      err.status = 404;
      throw err;
    }
